...
def summary_structure_prompt(key_value_pairs):
...
def articles_structure_prompt(summary):
...
def articles_generate(keywords):
    all_articles = {}
    ...
    for keyword in keywords:
        ...
        for pmid in pmids:
            ...
            article = articles_extract(article_url)
            if article:
...
def summarize():
    ...
    if key_values:
        prompt = summary_structure_prompt(key_values)
    else:
...
def articles():
    ...
    prompt = articles_structure_prompt(summary)
    ...
    try:
        ...
        if response:
            ...
            return jsonify({"articles": articles_generate(keywords)}), 200
        else:
